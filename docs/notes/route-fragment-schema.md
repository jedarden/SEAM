...
- **`x-vault-path` has the co-owned shape `<base>/<x-seam-owner>/<name>`** — lowercase-hyphen segments, at least one base segment above the owner and a name below it, with traversal, globs, and templated segments rejected at schema level. The base prefix is deliberately *not* encoded: it is deployment configuration (`SEAM_VAULT_BASE_DIR`), and pinning a literal would turn every base move (e.g. the consolidation onto `secret/rs-manager/rs-manager/seam/routes/*`) into a schema release. The schema and `seam lint` check the relative half — the owner appears as an interior segment; the runtime enforcer (`AllowlistEnforcer.ValidateVaultPath`) checks the configured base and that the owner segment equals `x-seam-owner`.
