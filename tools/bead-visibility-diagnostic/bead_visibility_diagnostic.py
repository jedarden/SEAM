...
class BeadVisibilityChecker:
    ...
    def query_cli_open(self) -> List[str]:
        ...
        try:
            result = subprocess.run(
                ["/home/coding/.local/bin/bead", "list", "--status", "open", "--json"],
                cwd=self.workspace,
                capture_output=True,
                text=True,
                timeout=30
    ...
    def query_cli_ready(self) -> List[str]:
        ...
        try:
            result = subprocess.run(
                ["/home/coding/.local/bin/bead", "list", "--ready", "--json"],
                cwd=self.workspace,
                capture_output=True,
                text=True,
                timeout=30
    ...
    def flush_checkpoint(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["/home/coding/.local/bin/bead", "sync", "flush-only"],
                cwd=self.workspace,
                capture_output=True,
                text=True,
                timeout=60
    ...
    def detect_and_repair(self) -> Dict[str, Any]:
        ...
        if len(stuck_assigned) > 0:
            ...
            for bead in stuck_assigned[:10]:  # Limit to 10 to avoid spam
                ...
                try:
                    result = subprocess.run(
                        ["/home/coding/.local/bin/bead", "release", bead_id],
                        cwd=self.workspace,
                        capture_output=True,
                        text=True,
                        timeout=30
